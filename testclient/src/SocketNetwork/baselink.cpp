...
INT32 baselink::ConnectToLoginServer()
{
//    if (!Init(-2)) {
//        std::cout << "login server connecting with db server: \n"
...
//    }
    ...
    if (m_socketfd < 0)
    {
        std::cout << "Client connecting with login server: \n"
                     "socket fd is wrong! \n" <<
                     "errno:" << errno << ".\n"
                     << std::endl;
        return -1;
    ...
    if (connect(m_socketfd, (sockaddr*)&serveraddr, sizeof(serveraddr)) < 0)
    {
        std::cout << "Client connecting with login server: "
                     ...
                     << std::endl;
        perror("connect login server:" );
        return -1;
    }

    std::cout << "Client connecting with login server: connect success. \n" << std::endl;
    return m_socketfd;
}
